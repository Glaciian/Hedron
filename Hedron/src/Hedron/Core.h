...
// Definitions for Windows platforms.
	...
	#ifdef HDE_BUILD_DLL
		// Define HEDRON_API as __declspec(dllexport), indicating that the symbols 
		// following this macro are being exported from the DLL.
		#define HEDRON_API __declspec(dllexport) 
	#else
		// If HDE_BUILD_DLL is not defined, define HEDRON_API as __declspec(dllimport).
		// This indicates that the symbols following this macro are being imported from the DLL.
